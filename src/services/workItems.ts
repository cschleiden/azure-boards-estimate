...
export class WorkItemService implements IWorkItemService {
    async getWorkItems(workItemIds: number[]): Promise<IWorkItem[]> {
        ...
        const workItems = await workItemTrackingClient.getWorkItemsBatch({
            ids: workItemIds,
            fields: ["System.Id", "System.Title", "System.WorkItemType", "System.TeamProject"],
        ...
        for (const workItem of mappedWorkItems) {
            if (foo.has(workItem.project)) {
                ...
                foo.set(workItem.project, {
                    workItemTypes: new Map<string, string>([[workItem.workItemType, ""]])
                });
            }
        }
        ...
        await Promise.all(Array.from(foo.entries()).map(async ([projectName, projectInfo]) => {
            ...
            // Get process type id
            const properties = await coreClient.getProjectProperties(project.id, ["System.ProcessTemplateType"]);
            ...
            await Promise.all(Array.from(projectInfo.workItemTypes.keys()).map(async (workItemTypeName) => {
                ...
                projectInfo.workItemTypes.set(workItemTypeName, descriptionRefName);
            }));
        }));
        ...
        const workItemDescriptions = await workItemTrackingClient.getWorkItemsBatch({
            ids: workItemIds,
            fields: Array.prototype.concat.apply(null, Array.from(foo.values()).map(x => Array.from(x.workItemTypes.values()))),
            $expand: WorkItemExpand.None,
            errorPolicy: WorkItemErrorPolicy.Omit
        } as WorkItemBatchGetRequest);
        ...
        for (const workItemDescription of workItemDescriptions) {
            try {
                ...
                const descriptionRefName = foo.get(wi.project)!.workItemTypes.get(wi.workItemType)!;
