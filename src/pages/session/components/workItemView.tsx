  ...
  render() {
    ...
    return (
      <>
        ...
        {!revealed && (
          <>
            <PrimaryButton onClick={this.doReveal}>Reveal</PrimaryButton>
          </>
        )}
        {revealed && (
          <>
            <div>
              These were the cards selected, choose one to commit the value to
              the work item:
            </div>
            <div>
              {estimates.map(e =>
                this.renderCard(
                  cardSet.cards.find(x => x.identifier === e.cardIdentifier)!
                )
              )}
            </div>
            <div>
